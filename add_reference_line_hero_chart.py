...
for conc_level in concurrency_levels:
    ...
    print(f"Plotted {len(subset)} points for concurrency {conc_level}")

# ADD DIAGONAL REFERENCE LINE - Perfect response time (y = x)
x_line = np.linspace(1, 5, 100)  # From 1 to 5 seconds
plt.plot(x_line, x_line, 
         linestyle='--', 
         color='gray', 
         alpha=0.7, 
         linewidth=2,
         label='Perfect response time (y=x)')

print("Added diagonal reference line showing perfect response time frontier")
...
# Customize the legend - position in lower-right, now including reference line
...
print("\n✅ Hero chart updated with diagonal reference line!")
print("📏 Added theoretical perfect response line (y=x):")
print("   - Shows where response time equals workload")
print("   - Dashed gray line for clear distinction")
print("   - Demonstrates n8n overhead under concurrency")
print("🎨 All features preserved:")
print("   - Blue color scheme (light to navy)")
print(f"   - All {len(df)} data points with 1-second workload")
print("   - Jitter, legend, styling unchanged")
